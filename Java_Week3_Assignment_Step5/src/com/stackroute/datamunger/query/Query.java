...
public class Query {
	 ...
	 * processor.
	 * 
	 * In this example, we are going to work with only one Query Processor which
	 * is CsvQueryProcessor, which can work with select queries containing zero,
	...
	public HashMap executeQuery(String queryString) {
		 ...
		 * assignment, we will process only queries containing zero, one or
		 * multiple where conditions i.e. conditions without aggregate
		 * functions, order by clause or group by clause
		 */
